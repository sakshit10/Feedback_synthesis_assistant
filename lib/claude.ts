...
function getClient(): Anthropic {
  ...
  if (!apiKey) {
    throw new Error(
      'ANTHROPIC_API_KEY is not set. Add it to your .env.local file (see .env.example).'
    );
  }
...
export async function runSynthesis(
  ...
  const response = await client.messages.create({
    ...
    messages: [{ role: 'user', content: buildUserPrompt(usedItems, notes) }],
  });

  const textBlock = response.content.find((b) => b.type === 'text');
  const rawModel = textBlock && 'text' in textBlock ? textBlock.text : '';
